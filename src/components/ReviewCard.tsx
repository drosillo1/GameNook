...
export default function ReviewCard({ review, currentUserId, isOwn: isOwnProp }: ReviewCardProps) {
  ...
  const ratingData = getRatingData(review.rating)
  const ratingIcon = ICON_MAP[ratingData.iconName as keyof typeof ICON_MAP]
  ...
  return (
    <div className={`border border-white/[0.06] rounded-xl bg-gn-card overflow-hidden
                     ${!isEditing ? 'flex flex-col md:flex-row' : ''}`}>

      {/* ── Rating Panel (solo en modo vista) ── */}
      {!isEditing && (
        <>
          {/* Desktop: panel lateral izquierdo */}
          <div
            className="hidden md:flex flex-col items-center justify-center w-[100px] flex-shrink-0
                       border-r border-white/[0.06]"
            style={{ backgroundColor: `${ratingData.color}0a` }}
          >
            <span
              className="font-black text-4xl leading-none tracking-[-0.05em]"
              style={{ color: ratingData.color, fontFamily: 'Orbitron, monospace' }}
            >
              {review.rating}
            </span>
            <span className="text-gn-muted text-[11px] mt-0.5">/10</span>
            <div className="mt-2.5" style={{ color: ratingData.color }}>
              <PxlKitIcon icon={ratingIcon} size={22} />
            </div>
            <span
              className="text-[10px] font-bold uppercase tracking-wider mt-1.5 text-center leading-tight px-2"
          ...
          <div
            className="flex md:hidden items-center gap-3 px-4 py-2.5 border-b border-white/[0.06]"
            style={{ backgroundColor: `${ratingData.color}0a` }}
          >
            <div className="flex items-baseline gap-0.5">
              ...
              <span className="text-gn-muted text-xs">/10</span>
            </div>
            <div style={{ color: ratingData.color }}>
              <PxlKitIcon icon={ratingIcon} size={16} />
            </div>
            <span
      ...
      <div className="flex-1 px-5 py-4 min-w-0">
        ...
        {isEditing ? (
          ...
          /* ── Modo vista: contenido de la reseña ── */
          review.content && <ExpandableContent content={review.content} />
        )}

        {/* Botón de like (solo en modo vista) */}
        {!isEditing && (
